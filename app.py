...
with st.sidebar:
    uploaded_file = st.file_uploader("Upload .csv or .xlsx file not excedding 10 MB")
...
if uploaded_file is not None:
    ...
    if ext:
        ...
        if filesize <= 10:
            ...
            st.error(f'Maximum allowed filesize is 10 MB. But received {filezise} MB')
    else:
        st.error('kindly upload only .csv or .xlsx only.')
